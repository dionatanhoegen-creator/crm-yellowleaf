  ...
  return (
    <div className="w-full p-4">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={() => { setEditingOp(null); setFormData({ cnpj: '', nomeCliente: '', contato: '', telefone: '', produto: '', aplicacao: '', valor: '', dataEntrada: new Date().toISOString().split('T')[0], estagio: 'prospeccao', dataLembrete: '', observacoes: '' }); setModalOpen(true); }} className="bg-[#2563eb] text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-lg transition active:scale-95">
      ...
      {modalOpen && mounted && createPortal(
        <div className="fixed inset-0 z-[999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95">
            ...
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-5 overflow-y-auto bg-white flex-1">
              ...
              <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">Valor (R$)</label><input type="number" className="w-full bg-slate-50 border rounded-xl p-3 font-black text-green-700" value={formData.valor} onChange={e => setFormData({...formData, valor: e.target.value})}/></div>
