...
export default function PipelinePage() {
  ...
  const carregarOportunidades = async () => {
    ...
    if (user) {
      ...
      // NOTA: Removi o .eq('user_id', user.id) temporariamente para garantir que você veja TUDO
      // Depois podemos filtrar de novo se quiser ver só os seus.
  ...
  const handleSave = async () => {
    ...
    // --- LÓGICA DE BLOQUEIO DE CNPJ REVISADA ---
    const cnpjLimpo = formData.cnpj.replace(/\D/g, ''); // 07218...
    // Cria formato XX.XXX.XXX/XXXX-XX manualmente para garantir
    const cnpjFormatado = cnpjLimpo.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})$/, "$1.$2.$3/$4-$5");

    // Só valida se tiver 14 dígitos (CNPJ completo)
    if (cnpjLimpo.length === 14) {
        // BUSCA HÍBRIDA: Procura tanto o limpo quanto o formatado
        ...
        if (clientesExistentes && clientesExistentes.length > 0) {
            // Filtra conflitos REAIS (Ignora o próprio registro se estiver editando)
