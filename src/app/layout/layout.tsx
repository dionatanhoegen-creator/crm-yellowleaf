...
export default function RootLayout({ children }: { children: React.ReactNode }) {
    ...
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker.getRegistrations().then(registrations => {
            for(let registration of registrations) {
                registration.unregister(); // Limpa lixo antigo
            }
        });
        navigator.serviceWorker.register('/sw.js?v=10').catch((err) => console.log('Erro no SW', err));
    }
  ...
  const handleInstallApp = async () => {
    if (!deferredPrompt) {
        alert("O Android ainda está processando o aplicativo. Aguarde 5 segundos e tente novamente.");
        return;
    }
  ...
  if (isLoginPage) {
    return (
      <html lang="pt-br">
        <head>
          <link rel="manifest" href="/manifest-v10.json" />
    ...
    <html lang="pt-br">
      ...
      <body className={`${outfit.className} bg-slate-50 text-slate-700 overflow-x-hidden`}>
        ...
        <aside className={`fixed top-0 left-0 h-full w-[80vw] max-w-[300px] sm:w-72 bg-[#0f392b] text-white z-[60] shadow-2xl transform transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <nav className="p-4 overflow-y-auto h-[calc(100vh-140px)] custom-scrollbar pb-24">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-[#0a261d] border-t border-white/5 flex flex-col gap-2">
            
            {/* NOSSO BOTÃO DE INSTALAR PWA NO ANDROID */}
            {deferredPrompt && (
