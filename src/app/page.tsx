...
export default function Dashboard() {
  ...
  const salvarTarefa = async () => {
    if (!formTarefa.titulo || !formTarefa.data || !formTarefa.hora) return alert("Campos obrigatórios: Título, Data e Hora.");
    const { data: { user } } = await supabase.auth.getUser();
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-8 pb-20">
      ...
      {/* 2. CARDS DE KPI COM BORDAS VERDES */}
          ...
          <div className="p-6 space-y-4 max-h-[500px] overflow-y-auto">
            {tarefas.length === 0 ? <p className="text-center py-10 text-slate-400 text-sm">Nenhuma tarefa pendente. ☀️</p> : tarefas.map(t => (
              <div key={t.id} className="group bg-slate-50 hover:bg-white hover:shadow-md border border-transparent hover:border-green-100 p-5 rounded-[2rem] transition-all flex items-center gap-6">
                <button className="shrink-0">{t.concluido ? <CheckCircle2 className="text-green-500" size={28}/> : <Circle className="text-slate-200" size={28}/>}</button>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[200] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-8 bg-slate-800 text-white flex justify-between items-center">
              <div><h2 className="text-2xl font-black italic tracking-tighter uppercase">{editingTarefa ? 'Editar Tarefa' : 'Nova Tarefa'}</h2><p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">YellowLeaf System</p></div>
              <button onClick={() => setIsTaskModalOpen(false)} className="bg-white/10 p-2 rounded-full transition hover:bg-white/20"><X size={24}/></button>
            </div>
            <div className="p-10 space-y-8 overflow-y-auto">
              <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">O que precisa ser feito? *</label>
              <input type="text" className="w-full bg-slate-50 border-2 border-slate-100 rounded-3xl p-5 font-bold outline-none focus:border-green-500 transition" value={formTarefa.titulo} onChange={e => setFormTarefa({...formTarefa, titulo: e.target.value})} /></div>
