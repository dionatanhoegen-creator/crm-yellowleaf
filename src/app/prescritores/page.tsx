      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-3 md:gap-4 mb-6 md:mb-8">
            <div className="relative flex-1">
              <input type="text" placeholder="Buscar por nome, CRM, clínica..." value={busca} onChange={(e) => setBusca(e.target.value)} className="w-full pl-12 pr-4 py-3.5 md:py-4 bg-white border border-slate-200 rounded-2xl shadow-sm focus:border-blue-500 outline-none text-base md:text-lg font-medium transition" />
        ...
        {loading && !modalInteracoes ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
            ...
            {filtrados.map((p) => {
              ...
              return (
              <div key={p.id} className={`bg-white rounded-2xl p-4 md:p-5 border shadow-sm transition group flex flex-col justify-between h-full relative ${alertaAtraso ? 'border-red-300 hover:shadow-red-100' : 'border-slate-100 hover:shadow-md hover:border-blue-200'}`}>
                 
                 {p.proximo_contato && (
                    <div className="absolute top-0 left-0 w-full flex justify-center -mt-2.5 md:-mt-3 z-10">
                        <span className={`px-2 md:px-3 py-0.5 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest shadow-sm flex items-center gap-1 md:gap-1.5 border ${alertaAtraso ? 'bg-red-500 text-white border-red-600 animate-pulse' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                            {alertaAtraso ? <AlertCircle size={10} className="md:w-3 md:h-3"/> : <Clock size={10} className="md:w-3 md:h-3"/>} 
                 ...
                 <div className="absolute top-3 md:top-4 right-3 md:right-4 z-10">
                     <span className={`px-1.5 md:px-2 py-0.5 md:py-1 rounded-md border text-[8px] md:text-[9px] font-black uppercase tracking-widest flex items-center gap-1 ${p.potencial === 'A' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : p.potencial === 'B' ? 'bg-blue-100 text-blue-700 border-blue-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                         <Star size={8} className={`md:w-2.5 md:h-2.5 ${p.potencial === 'A' ? 'fill-current' : ''}`}/> Curva {p.potencial}
                     </span>
                 </div>

                 <div className="flex gap-3 md:gap-4 items-start mb-3 md:mb-4 pr-16 md:pr-20 mt-3 md:mt-2">
                    <div className="w-12 h-12 md:w-14 md:h-14 bg-slate-800 text-white rounded-xl flex items-center justify-center font-black text-lg md:text-xl shrink-0 shadow-sm">
                        {p.nome.substring(0, 2).toUpperCase()}
                    </div>
                    <div className="min-w-0">
                        <h3 className="font-black text-base md:text-lg text-slate-800 leading-tight line-clamp-2" title={p.nome}>{p.nome}</h3>
                        <p className="text-xs md:text-sm font-bold text-blue-600 mt-0.5 md:mt-1 truncate">{p.especialidade || 'Clínico Geral'}</p>
                        <p className="text-[9px] md:text-[10px] text-slate-400 font-mono mt-0.5 truncate">{p.crm_crn || 'CRM/CRN N/D'}</p>
                    </div>
                 </div>

                 <div className="space-y-1.5 md:space-y-2 mb-4 flex-1">
                    {p.clinica && (
                        <p className="text-[11px] md:text-xs text-slate-600 flex items-start gap-1.5 md:gap-2">
                            <Building2 size={12} className="text-slate-400 shrink-0 mt-0.5 md:w-3.5 md:h-3.5"/> 
                            <span className="line-clamp-1" title={p.clinica}>{p.clinica}</span>
                        </p>
                    )}
                    {p.telefone && (
                        <p className="text-[11px] md:text-xs text-slate-600 flex items-center gap-1.5 md:gap-2">
                            <Phone size={12} className="text-slate-400 shrink-0 md:w-3.5 md:h-3.5"/> 
                            <a href={`tel:${p.telefone}`} className="hover:text-blue-600 font-medium">{p.telefone}</a>
                        </p>
                    )}
                 </div>

                 <div className="border-t border-slate-100 pt-3 md:pt-4 flex justify-between items-center mt-auto">
                     <button onClick={() => abrirInteracoes(p)} className={`text-[11px] md:text-xs font-bold px-3 md:px-4 py-2 rounded-xl transition flex items-center gap-1.5 md:gap-2 ${alertaAtraso ? 'bg-red-50 text-red-700 border border-red-200 hover:bg-red-600 hover:text-white' : 'bg-green-50 text-green-700 border border-green-100 hover:bg-green-600 hover:text-white'}`}>
                         <CalendarCheck size={14}/> Diário
                     </button>
                     <div className="flex items-center gap-1.5 md:gap-2">
                        {p.telefone && (
                            <a href={`https://wa.me/55${p.telefone.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="text-green-500 hover:text-green-600 transition-colors p-1.5 md:p-2 bg-green-50 rounded-lg" title="Abrir WhatsApp">
                                <MessageCircle size={14} className="md:w-4 md:h-4"/>
                            </a>
                        )}
                        <button onClick={() => abrirEdicao(p)} className="text-[11px] md:text-xs font-bold text-blue-600 bg-blue-50 border border-blue-100 px-3 md:px-4 py-2 rounded-xl hover:bg-blue-600 hover:text-white transition flex items-center gap-1.5 md:gap-2">
                            <Edit size={12} className="md:w-3.5 md:h-3.5"/> Editar
                        </button>
                     </div>
                 </div>
              </div>
        ...
        {modalAberto && mounted && !modalInteracoes && createPortal(
           <div className="fixed inset-0 z-[99999] flex items-end md:items-center justify-center bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-200 md:p-4 overflow-hidden">
             <div className="bg-white w-full h-[95vh] md:h-auto md:max-h-[90vh] md:max-w-3xl rounded-t-3xl md:rounded-[2rem] shadow-2xl flex flex-col animate-in slide-in-from-bottom-10 md:zoom-in-95 duration-200">
                
                <div className="bg-[#1e293b] p-5 md:p-6 flex justify-between items-center text-white shrink-0 relative overflow-hidden">
                   <div className="absolute -right-4 -top-4 text-white/5 transform rotate-12 pointer-events-none"><User size={120} className="md:w-[150px] md:h-[150px]"/></div>
                   <div className="relative z-10 min-w-0 pr-4">
                       <span className="bg-blue-500 text-white px-2 py-0.5 md:px-3 md:py-1 rounded text-[9px] md:text-[10px] font-black uppercase tracking-widest mb-1.5 md:mb-3 inline-block shadow-sm">Gestão de Base</span>
                       <h2 className="text-xl md:text-2xl font-black tracking-tight truncate">{form.id ? 'Editar Prescritor' : 'Cadastrar Prescritor'}</h2>
                   </div>
                ...
                <form onSubmit={handleSalvarPrescritor} className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 flex flex-col bg-slate-50">
                    <div className="flex-1">
                        <h3 className="text-base md:text-lg font-black text-slate-800 mb-3 md:mb-4 flex items-center gap-2 border-b border-slate-200 pb-2 md:pb-3">
                            <User className="text-blue-600" size={18}/> Profissional
                        </h3>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 mb-5 md:mb-6">
                            <div>
                                <label className="text-xs md:text-sm font-bold text-slate-700 mb-1.5 block">Nome Completo *</label>
                                <input required type="text" placeholder="Ex: Dr. Carlos Silva" value={form.nome} onChange={e => setForm({...form, nome: e.target.value})} className="w-full p-3 md:p-3.5 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm md:text-base font-medium text-slate-900 placeholder-slate-400 shadow-sm transition"/>
                            </div>
                            <div>
                                <label className="text-xs md:text-sm font-bold text-slate-700 mb-1.5 block">CRM / CRN</label>
                                <input type="text" placeholder="Ex: CRM-SP 123456" value={form.crm_crn} onChange={e => setForm({...form, crm_crn: e.target.value})} className="w-full p-3 md:p-3.5 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm md:text-base font-medium text-slate-900 placeholder-slate-400 shadow-sm transition"/>
                            </div>
                            
                            <div className="sm:col-span-2">
                                <label className="text-xs md:text-sm font-bold text-slate-700 mb-1.5 block">Especialidade Principal</label>
                                <input list="lista-especialidades" type="text" placeholder="Selecione na lista ou digite..." value={form.especialidade} onChange={e => setForm({...form, especialidade: e.target.value})} className="w-full p-3 md:p-3.5 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm md:text-base font-medium text-slate-900 placeholder-slate-400 shadow-sm transition"/>
                            ...
                            <div className="grid grid-cols-2 gap-3 sm:col-span-2">
                                <div>
                                    <label className="text-xs md:text-sm font-bold text-slate-700 mb-1.5 block">Curva (Potencial)</label>
                                    <select value={form.potencial} onChange={e => setForm({...form, potencial: e.target.value})} className="w-full p-3 md:p-3.5 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm md:text-base font-medium text-slate-900 cursor-pointer shadow-sm">
                                ...
                                <div>
                                    <label className="text-xs md:text-sm font-bold text-slate-700 mb-1.5 block">Perfil</label>
                                    <select value={form.perfil_prescritor} onChange={e => setForm({...form, perfil_prescritor: e.target.value})} className="w-full p-3 md:p-3.5 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm md:text-base font-medium text-slate-900 cursor-pointer shadow-sm">
                        ...
                        <h3 className="text-base md:text-lg font-black text-slate-800 mb-3 md:mb-4 flex items-center gap-2 border-b border-slate-200 pb-2 md:pb-3">
                            <Building2 className="text-blue-600" size={18}/> Localização
                        </h3>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 mb-5 md:mb-6">
                            <div>
                                <label className="text-xs md:text-sm font-bold text-slate-700 mb-1.5 block">Clínica / Consultório</label>
                                <input type="text" placeholder="Ex: Clínica Saúde" value={form.clinica} onChange={e => setForm({...form, clinica: e.target.value})} className="w-full p-3 md:p-3.5 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm md:text-base font-medium text-slate-900 placeholder-slate-400 shadow-sm transition"/>
                            </div>
                            <div>
                                <label className="text-xs md:text-sm font-bold text-slate-700 mb-1.5 block">WhatsApp / Secretária</label>
                                <input type="text" placeholder="(00) 00000-0000" value={form.telefone} onChange={e => setForm({...form, telefone: e.target.value})} className="w-full p-3 md:p-3.5 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm md:text-base font-medium text-slate-900 placeholder-slate-400 shadow-sm transition"/>
                            </div>
                            <div className="sm:col-span-2">
                                <label className="text-xs md:text-sm font-bold text-slate-700 mb-1.5 block">Endereço Completo</label>
                                <input type="text" placeholder="Rua, Número, Sala/Andar..." value={form.endereco} onChange={e => setForm({...form, endereco: e.target.value})} className="w-full p-3 md:p-3.5 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm md:text-base font-medium text-slate-900 placeholder-slate-400 shadow-sm transition"/>
                            </div>
                            <div className="grid grid-cols-12 gap-3 sm:col-span-2">
                                <div className="col-span-12 sm:col-span-5">
                                    <label className="text-xs md:text-sm font-bold text-slate-700 mb-1.5 block">Bairro</label>
                                    <input type="text" placeholder="Bairro" value={form.bairro} onChange={e => setForm({...form, bairro: e.target.value})} className="w-full p-3 md:p-3.5 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm md:text-base font-medium text-slate-900 placeholder-slate-400 shadow-sm transition"/>
                                </div>
                                <div className="col-span-8 sm:col-span-5">
                                    <label className="text-xs md:text-sm font-bold text-slate-700 mb-1.5 block">Cidade</label>
                                    <input type="text" placeholder="Cidade" value={form.cidade} onChange={e => setForm({...form, cidade: e.target.value})} className="w-full p-3 md:p-3.5 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm md:text-base font-medium text-slate-900 placeholder-slate-400 shadow-sm transition"/>
                                </div>
                                <div className="col-span-4 sm:col-span-2">
                                    <label className="text-xs md:text-sm font-bold text-slate-700 mb-1.5 block">UF</label>
                                    <input type="text" placeholder="UF" value={form.uf} onChange={e => setForm({...form, uf: e.target.value})} maxLength={2} className="w-full p-3 md:p-3.5 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm md:text-base font-bold uppercase text-center text-slate-900 placeholder-slate-400 shadow-sm transition"/>
                                </div>
                            </div>
                        </div>

                        <h3 className="text-base md:text-lg font-black text-slate-800 mb-3 md:mb-4 flex items-center gap-2 border-b border-slate-200 pb-2 md:pb-3">
                            <AlignLeft className="text-blue-600" size={18}/> Anotações
                        </h3>
                        <div className="mb-4">
                            <label className="text-xs md:text-sm font-bold text-slate-700 mb-1.5 block">Observações Gerais</label>
                            <textarea rows={3} value={form.observacoes} onChange={e => setForm({...form, observacoes: e.target.value})} className="w-full p-3 md:p-4 border border-slate-300 rounded-xl bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm md:text-base font-medium text-slate-900 placeholder-slate-400 shadow-sm resize-none transition" placeholder="Perfil de prescrição, concorrentes, hobbies..."></textarea>
                        </div>
                    </div>

                    <div className="flex flex-col-reverse sm:flex-row gap-3 pt-4 border-t border-slate-200 shrink-0 mt-auto pb-4 sm:pb-0">
                        {form.id && (
                            <button type="button" onClick={handleExcluirPrescritor} disabled={excluindo} className="w-full sm:w-auto px-5 py-3.5 sm:py-3 text-red-600 font-bold bg-white sm:bg-red-50 border border-red-200 sm:border-transparent hover:bg-red-100 rounded-xl transition text-sm flex items-center justify-center gap-2 disabled:opacity-50">
                        ...
                        <div className="flex-1 flex flex-col-reverse sm:flex-row gap-3 sm:justify-end">
                            <button type="button" onClick={() => setModalAberto(false)} className="w-full sm:w-auto px-6 py-3.5 sm:py-3 text-slate-600 font-bold bg-white sm:bg-slate-100 border border-slate-200 sm:border-transparent hover:bg-slate-200 rounded-xl transition text-sm">Cancelar</button>
                            <button type="submit" disabled={salvando} className="w-full sm:w-auto px-8 bg-blue-600 text-white py-3.5 sm:py-3 rounded-xl font-black hover:bg-blue-700 shadow-lg shadow-blue-200 transition transform active:scale-95 disabled:opacity-50 text-sm flex justify-center items-center gap-2">
