             ...
             <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[95vh]">
                ...
                <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
                    ...
                    <div className="md:w-1/2 p-6 bg-slate-50 overflow-y-auto custom-scrollbar border-r border-slate-200">
                        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2 border-b border-slate-200 pb-2"><FileText size={14}/> Histórico de Interações</h3>
                                ...
                                interacoes.map((int) => (
                                    <div key={int.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative group hover:border-blue-200 transition">
                                        <div className="flex justify-between items-start mb-3">
                                            <span className="text-[10px] font-black text-blue-700 bg-blue-50 border border-blue-100 px-2 py-1 rounded uppercase tracking-wider">{int.tipo}</span>
                                            <span className="text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-1 rounded">{new Date(int.created_at).toLocaleDateString('pt-BR')}</span>
                                        </div>
                                        <p className="text-sm text-slate-700 font-medium mb-4 whitespace-pre-wrap leading-relaxed">{int.resumo}</p>
                                        
                                        {int.proximo_passo && (
                                            <div className="bg-green-50/50 p-3 rounded-xl border border-green-100 flex items-start gap-2 mb-3">
                                                <ChevronRight size={16} className="text-green-600 mt-0.5 shrink-0"/>
                                                <p className="text-xs text-slate-700 font-medium"><span className="text-green-800 font-bold uppercase text-[10px] tracking-wider block mb-0.5">Próximo Passo</span> {int.proximo_passo}</p>
                                            </div>
                                        )}
                                        <p className="text-[9px] text-slate-400 uppercase tracking-widest text-right mt-2 border-t border-slate-50 pt-3">
                    ...
                    <div className="md:w-1/2 p-6 bg-white overflow-y-auto custom-scrollbar">
                        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2 border-b border-slate-100 pb-2"><Plus size={14}/> Registrar Nova Visita</h3>
                        
                        <form onSubmit={salvarInteracao} className="space-y-5">
                            <div>
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Tipo de Contato</label>
                                <select value={novaInteracao.tipo} onChange={e => setNovaInteracao({...novaInteracao, tipo: e.target.value})} className="w-full p-4 border-2 border-slate-100 rounded-xl text-sm font-bold bg-slate-50 focus:border-blue-500 focus:bg-white outline-none transition">
                            ...
                            <div>
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Resumo / Ativos Apresentados *</label>
                                <textarea required rows={5} value={novaInteracao.resumo} onChange={e => setNovaInteracao({...novaInteracao, resumo: e.target.value})} className="w-full p-4 border-2 border-slate-100 rounded-xl text-sm font-medium bg-slate-50 focus:border-blue-500 focus:bg-white outline-none resize-none transition" placeholder="Descreva os protocolos discutidos, feedback do médico sobre os produtos..."></textarea>
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Próximo Passo (Ação futura)</label>
                                <input type="text" value={novaInteracao.proximo_passo} onChange={e => setNovaInteracao({...novaInteracao, proximo_passo: e.target.value})} className="w-full p-4 border-2 border-slate-100 rounded-xl text-sm font-medium bg-slate-50 focus:border-blue-500 focus:bg-white outline-none transition" placeholder="Ex: Enviar amostra física dia 20/05"/>
                            </div>
                            <div className="pt-4 border-t border-slate-100 mt-6">
                                <button type="submit" disabled={salvando} className="w-full bg-blue-600 text-white h-14 rounded-xl font-black hover:bg-blue-700 flex justify-center items-center gap-2 shadow-lg shadow-blue-200 transition transform active:scale-95 disabled:opacity-50 uppercase tracking-wide text-xs">
                                    <Save size={18}/> {salvando ? 'Salvando...' : 'Gravar Histórico'}
                                </button>
                            </div>
                        </form>
                    </div>
           ...
           <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-200 overflow-y-auto">
             <div className="bg-white w-full max-w-3xl rounded-[2rem] shadow-2xl overflow-hidden flex flex-col my-8">
                ...
                <form onSubmit={handleSalvarPrescritor} className="p-6 md:p-8">
                    ...
                    <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2 border-b border-slate-100 pb-2"><User size={14}/> Informações Profissionais</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                        <div>
                            ...
                            <input required type="text" placeholder="Ex: Dr. Carlos Silva" value={form.nome} onChange={e => setForm({...form, nome: e.target.value})} className="w-full p-3 border-2 border-slate-100 rounded-xl bg-slate-50 focus:border-blue-500 focus:bg-white outline-none font-bold text-slate-800 transition"/>
                        </div>
                        <div>
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">CRM / CRN</label>
                            <input type="text" placeholder="Ex: CRM-SP 123456" value={form.crm_crn} onChange={e => setForm({...form, crm_crn: e.target.value})} className="w-full p-3 border-2 border-slate-100 rounded-xl bg-slate-50 focus:border-blue-500 focus:bg-white outline-none font-medium transition"/>
                        </div>
                        <div>
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Especialidade Principal</label>
                            <input type="text" placeholder="Ex: Dermatologia, Nutrição Esportiva..." value={form.especialidade} onChange={e => setForm({...form, especialidade: e.target.value})} className="w-full p-3 border-2 border-slate-100 rounded-xl bg-slate-50 focus:border-blue-500 focus:bg-white outline-none font-medium transition"/>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Curva (Potencial)</label>
                                <select value={form.potencial} onChange={e => setForm({...form, potencial: e.target.value})} className="w-full p-3 border-2 border-slate-100 rounded-xl bg-slate-50 focus:border-blue-500 outline-none font-bold text-slate-700 cursor-pointer">
                            ...
                            <div>
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Perfil</label>
                                <select value={form.perfil_prescritor} onChange={e => setForm({...form, perfil_prescritor: e.target.value})} className="w-full p-3 border-2 border-slate-100 rounded-xl bg-slate-50 focus:border-blue-500 outline-none font-medium text-slate-700 cursor-pointer">
                    ...
                    <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2 border-b border-slate-100 pb-2"><Building2 size={14}/> Dados de Contato e Localização</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                        <div>
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Nome da Clínica / Consultório</label>
                            <input type="text" placeholder="Ex: Clínica Saúde Vita" value={form.clinica} onChange={e => setForm({...form, clinica: e.target.value})} className="w-full p-3 border-2 border-slate-100 rounded-xl bg-slate-50 focus:border-blue-500 focus:bg-white outline-none font-medium transition"/>
                        </div>
                        <div>
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Telefone / WhatsApp (Secretária)</label>
                            <input type="text" placeholder="(00) 00000-0000" value={form.telefone} onChange={e => setForm({...form, telefone: e.target.value})} className="w-full p-3 border-2 border-slate-100 rounded-xl bg-slate-50 focus:border-blue-500 focus:bg-white outline-none font-medium transition"/>
                        </div>
                        <div className="md:col-span-2">
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Endereço Completo</label>
                            <input type="text" placeholder="Rua, Número, Sala/Andar..." value={form.endereco} onChange={e => setForm({...form, endereco: e.target.value})} className="w-full p-3 border-2 border-slate-100 rounded-xl bg-slate-50 focus:border-blue-500 focus:bg-white outline-none font-medium transition"/>
                        </div>
                        <div className="grid grid-cols-12 gap-3 md:col-span-2">
                            <div className="col-span-5 md:col-span-4">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Bairro</label>
                                <input type="text" placeholder="Bairro" value={form.bairro} onChange={e => setForm({...form, bairro: e.target.value})} className="w-full p-3 border-2 border-slate-100 rounded-xl bg-slate-50 focus:border-blue-500 focus:bg-white outline-none font-medium transition"/>
                            </div>
                            <div className="col-span-5 md:col-span-6">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Cidade</label>
                                <input type="text" placeholder="Cidade" value={form.cidade} onChange={e => setForm({...form, cidade: e.target.value})} className="w-full p-3 border-2 border-slate-100 rounded-xl bg-slate-50 focus:border-blue-500 focus:bg-white outline-none font-medium transition"/>
                            </div>
                            <div className="col-span-2 md:col-span-2">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">UF</label>
                                <input type="text" placeholder="UF" value={form.uf} onChange={e => setForm({...form, uf: e.target.value})} maxLength={2} className="w-full p-3 border-2 border-slate-100 rounded-xl bg-slate-50 focus:border-blue-500 focus:bg-white outline-none font-bold uppercase text-center transition"/>
                            </div>
                        </div>
                    </div>

                    <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2 border-b border-slate-100 pb-2"><AlignLeft size={14}/> Anotações de P&D</h3>
                    <div className="mb-8">
                        <textarea rows={3} value={form.observacoes} onChange={e => setForm({...form, observacoes: e.target.value})} className="w-full p-4 border-2 border-slate-100 rounded-xl bg-slate-50 focus:border-blue-500 focus:bg-white outline-none font-medium resize-none transition" placeholder="Perfil de prescrição, concorrentes que utiliza, hobbies para relacionamento..."></textarea>
                    </div>

                    <div className="flex gap-4 pt-4 border-t border-slate-100">
                        <button type="button" onClick={() => setModalAberto(false)} className="flex-1 py-4 text-slate-500 font-bold hover:bg-slate-50 rounded-xl transition">Cancelar</button>
                        <button type="submit" disabled={salvando} className="flex-1 bg-blue-600 text-white py-4 rounded-xl font-black hover:bg-blue-700 shadow-lg shadow-blue-200 transition transform active:scale-95 disabled:opacity-50 uppercase tracking-wider text-xs flex justify-center items-center gap-2">
                           {salvando ? <Activity className="animate-spin" size={18}/> : <Save size={18}/>} {salvando ? 'Salvando...' : 'Gravar Ficha do Prescritor'}
                        </button>
                    </div>
                </form>
             </div>
